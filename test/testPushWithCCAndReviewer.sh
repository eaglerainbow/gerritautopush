...
. ./infra.inc
TESTDIR=pushWithCCAndReviewer

echo "*** Running test Push including multiple reviewers and CC users"
...
$SUBJECT -s -a -c -m "Change-Id:  dummy commit message" -p origin -b master -v reviewer@example.bogus -v reviewer2@example.bogus -o ccuser@example.bogus -o ccuser2@example.bogus -g $PWD/../../git_push_cc_and_reviewer.sh
